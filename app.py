...
def get_data(estacion, dias=1):
    # Especificamos columnas explícitas para evitar errores de sintaxis
    query = f"""
        SELECT timestamp, temperatura, precipitacion, humedad, voltaje_bateria, estado_bateria, presion, velocidad_del_viento
...
df_full = get_data(seleccion, dias=mapa_dias[periodo])
row = df_full.iloc[0] if not df_full.empty else None
...
if row is not None:
    ...
    with tab1:
        st.markdown("### 🌡️ Tendencia Temperatura")
        st.line_chart(df_full.set_index('timestamp')[['temperatura']])
        st.markdown("### 🌧️ Tendencia Precipitación")
        st.line_chart(df_full.set_index('timestamp')[['precipitacion']])
    with tab2:
